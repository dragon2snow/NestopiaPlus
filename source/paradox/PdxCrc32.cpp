...
const ULONG PDXCRC32::table[256] = 
{
...
ULONG PDX_STDCALL PDXCRC32::ComputeCrc(const UCHAR* const block,const TSIZE size)
{
	PDX_ASSERT(block || !size);

	ULONG crc = 0xFFFFFFFFUL;

	for (TSIZE i=0; i < size; ++i)
		crc = (crc >> 8) ^ table[(crc & 0xFF) ^ block[i]];
...
ULONG PDX_STDCALL PDXCRC32::UpdateCrc(const UCHAR* const block,const TSIZE size,ULONG crc)
{
	PDX_ASSERT(block || !size);
