...
namespace Nestopia
{
	...
	const MsgHandler::Entry<Paths> Paths::Handlers::commands[] =
	{
		...
		{ IDC_PATHS_CANCEL,             &Paths::OnCmdCancel      }
	};
	...
	{
		ibool unchecked = dialog.CheckBox( IDC_PATHS_IMAGE_LAST ).IsUnchecked();
		dialog.Control( IDC_PATHS_IMAGE ).Enable( unchecked );
		dialog.Control( IDC_PATHS_IMAGE_BROWSE ).Enable( unchecked );

		unchecked = dialog.CheckBox( IDC_PATHS_NSP_LAST ).IsUnchecked();
	...
	ibool Paths::OnCmdCancel(Param& param)
	{
		if (param.Button().IsClicked())
			dialog.Close();
