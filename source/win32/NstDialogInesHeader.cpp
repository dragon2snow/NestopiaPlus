...
namespace Nestopia
{
	...
	const MsgHandler::Entry<InesHeader> InesHeader::Handlers::commands[] =
	{
		...
		{ IDC_INES_HEADER_CANCEL,   &InesHeader::OnCmdCancel   },
	...
	ibool InesHeader::Save(Header& saveHeader) const
	{
		...
		if (dialog.CheckBox( IDC_INES_HEADER_BATTERY ).IsChecked())
			flags |= Header::BATTERY;

		if (dialog.CheckBox( IDC_INES_HEADER_TRAINER ).IsChecked())
			flags |= Header::TRAINER;

		if (dialog.CheckBox( IDC_INES_HEADER_VSSYSTEM ).IsChecked())
			flags |= Header::VS;

		if (dialog.RadioButton( IDC_INES_HEADER_VERTICAL ).IsChecked())
		{
			flags |= Header::VERTICAL;
		}
		else if (dialog.RadioButton( IDC_INES_HEADER_FOURSCREEN ).IsChecked())
		{
		...
		if (dialog.CheckBox( IDC_INES_HEADER_PAL ).IsChecked())
	...
	ibool InesHeader::OnCmdSave(Param& param)
	{
		if (param.Button().IsClicked())
		{
			...
			if (IsBad( saveHeader, imageSize ) && !User::Confirm( IDS_INES_HEADER_UNSAFE ))
				return true;
