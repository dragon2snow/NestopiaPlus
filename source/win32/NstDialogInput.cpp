...
namespace Nestopia
{
	...
	const Input::Settings::Mapping Input::Settings::map[NUM_KEYS] =
	{
		...
		{ EMULATION_KEYS + EMULATION_KEY_ALT_SPEED,       DIK_BACK, "Alternative Speed", "input emulation alternative speed" },
	...
	void Input::UpdateKeyNames(const uint type)
	{
	...
	void Input::UpdateKeyMap(const uint type)
	{
	...
	{
		const BYTE* const buffer = directInput.GetKeyboardBuffer();
