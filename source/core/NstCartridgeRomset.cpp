...
namespace Nes
{
	namespace Core
	{
		...
		)
		{
			...
			{
				Profiles profiles;
				Collect( stdStreamImage, favoredSystem, profiles );
				ChooseProfile( favoredSystem, askProfile, profiles, profile );
			}

			LoadRoms( prg, chr, profile, readOnly );

			if (stdStreamIps)
			{
				Ips ips;

				if (ips.Load( stdStreamIps ))
				{
					if (ips.Patch( prg.Mem(), prg.Size(), 16 ))
						Log::Flush( "Romset: PRG-ROM was IPS patched" NST_LINEBREAK );

					if (ips.Patch( chr.Mem(), chr.Size(), 16 + prg.Size() ))
						Log::Flush( "Romset: CHR-ROM was IPS patched" NST_LINEBREAK );
				}
				else
				{
					Log::Flush( "Romset: Warning, invalid or corrupt IPS file" NST_LINEBREAK );
				}
			}
		}

		void Cartridge::Romset::Collect(StdStream const stream,const FavoredSystem favoredSystem,Profiles& profiles)
		{
			Xml xml;

			if (!xml.Read( *static_cast<std::istream*>(stream) ))
		...
		void Cartridge::Romset::ChooseProfile
		(
			const FavoredSystem favoredSystem,
			const bool askProfile,
		...
		void Cartridge::Romset::LoadRoms(Ram& prg,Ram& chr,Profile& profile,const bool readOnly)
		{
			for (uint i=0; i < 2; ++i)
			{
