snes_ntsc 0.2.1: SNES NTSC Video Filter
---------------------------------------
Snes_ntsc emulates SNES NTSC video output, allowing an authentic image in an
...
emulator's frame rate. Custom blitters can be easily written using a special
...
10-year-old 400 MHz G3 PowerMac. The library requires 4MB of memory.
...
If you're using C++, to avoid linking issues rename snes_ntsc.c to
snes_ntsc.cpp (this causes compilation in C++ rather than C).

See snes_ntsc.txt for more information, and respective header (.h) files for
